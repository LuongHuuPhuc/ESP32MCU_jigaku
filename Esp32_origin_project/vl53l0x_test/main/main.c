...
void i2c_legacy_init(){
  ...
  // ESP_LOGI(TAG, "Xoa driver cu (neu co)...");
  // if(i2c_driver_delete(I2C_PORT) != ESP_ERR_INVALID_STATE){
  //   ESP_LOGW(TAG, "Driver I2C cu da duoc xoa !");
  // }
...
void app_main(){
  ...
  if(scan_i2c_device(VL53L0X_ADDR >> 1) == ESP_OK){
    ESP_LOGI(TAG, "Phat hien cam bien vl53l0x tai dia chi 0x52");
